...
void SolarSystemGame::Initialize()
{
    ...
    CosmicBody* uranus_ring = new CosmicBody(uranus_ring_rad, 0.8f, DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f), DirectX::XMFLOAT4(0.1f, 0.1f, 1.0f, 1.0f), CosmicBody::PLANET_TYPE::RING,
        uranus, 0.0f, 5.1f);
    uranus_ring->orbitAxis = Vector3(0.0f, cosf(XM_PIDIV2 * 0.9), sinf(XM_PIDIV2 * 0.9));
    ...
    focusedBody = moon;
    //renderer.camera.SwitchToOrbitalMode(focusedBody->GetCenterLocation(), focusedBody->orbitAxis, focusedBody->radius);
...
void SolarSystemGame::Update(float deltaTime)
{
    ...
    else if (inputHandler->IsKeyDown(InputHandler::KeyCode::B))
    {
