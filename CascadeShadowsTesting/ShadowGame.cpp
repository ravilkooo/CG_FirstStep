...
ShadowGame::ShadowGame()
{
	...
	lightData.pointLight.Ambient = { 0, 0, 0, 1 };
	...
	lightData.pointLight.Att = { 0.01f, 0.5f, 0.0f };


	light_pcb = new Bind::PixelConstantBuffer<LightData>(renderer.GetDevice(), lightData, 1u);
	...
	lightViewCamera->SetTarget({ 0.0, 0.0, 1.0f });
	//lightViewCamera->SwitchToOrbitalMode
	...
	auto _tb = new Bind::TextureB(renderer.GetDevice(), shadowTexture, depthSRV);

	TestCube* _tc = new TestCube(renderer.GetDevice(), 0.2, 0.2, 0.2, { 0,0,0 }, { 1,0,0,1 }, _tb);
	_tc->SetInitTransform(Matrix::CreateFromYawPitchRoll(XM_PIDIV4, XM_PIDIV4, 0));
	scene.AddNode(_tc);

	TestCube* _tc_2 = new TestCube(renderer.GetDevice(), 0.2f, 0.2f, 0.2f, { 0.5f, 0.0f, 0.0f }, { 0.9f, 0.7f, 0.5f, 1.0f }, _tb);
	_tc_2->SetInitTransform(Matrix::CreateFromYawPitchRoll(XM_PIDIV2 * 0.4, XM_PIDIV4 * 0.12, 0));
	scene.AddNode(_tc_2);

	TestCube* _tc_3 = new TestCube(renderer.GetDevice(), 10.0f, 10.0f, 1.0f, { 0.0f, 0.0f, 5.0f }, { 0.3f, 0.7f, 0.5f, 1.0f }, _tb);
	scene.AddNode(_tc_3);
	...
	shadowTransforms = new Bind::VertexConstantBuffer<ShadowTransform>(renderer.GetDevice(), 0u);
...
void ShadowGame::Update(float deltaTime)
{
	...
	for (TestCube* node : shadowableObjects) {
		node->shadowPixelCBuffer->Update(renderer.GetDeviceContext(),
			{
					node->worldMat, lightViewCamera->GetViewMatrix() * lightViewCamera->GetProjectionMatrix()
			});
	}
...
void ShadowGame::DrawSceneToShadowMap() {
	...
	for (SceneNode* node : scene.nodes) {
		...
		shadowTransforms->Update(renderer.GetDeviceContext(),
			{
				node->worldMat, lightViewCamera->GetViewMatrix() * lightViewCamera->GetProjectionMatrix()
			});
